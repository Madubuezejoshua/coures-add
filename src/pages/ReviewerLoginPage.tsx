  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!reviewerDoc.exists()) {
        await auth.signOut();
        setError('This account does not have reviewer privileges');
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md">
        <div className="bg-slate-800/50 border border-slate-700/50 backdrop-blur-sm rounded-2xl shadow-2xl p-8">
          ...
          <h1 className="text-3xl font-bold text-white text-center mb-2">Reviewer Login</h1>
          <p className="text-slate-400 text-center mb-8">Sign in to your reviewer account</p>
          ...
          <div className="mt-8 pt-6 border-t border-slate-700/50">
            <p className="text-slate-400 text-sm text-center">
              ...
              <Link to="/reviewer/signup" className="text-amber-400 hover:text-amber-300 font-medium transition-colors">
                Sign up
              </Link>
            </p>
